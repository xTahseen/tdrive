...
#preview-bar {
  display: flex; align-items: center; gap: 10px;
  padding: 10px 14px; background: var(--header); border-bottom: 1px solid var(--border2);
  flex-shrink: 0; min-height: 52px;
}
#preview-title {
  flex: 1; min-width: 0; font-size: 15px; font-weight: 500; color: var(--text);
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
#preview-dl-btn {
  display: flex; align-items: center; gap: 6px;
  padding: 7px 14px; border-radius: var(--r8); border: none; cursor: pointer;
  background: var(--accent); color: #fff; font-size: 13px; font-weight: 600;
  white-space: nowrap; flex-shrink: 0;
}
#preview-dl-btn:hover { filter: brightness(1.12); }
#preview-close-btn {
  width: 34px; height: 34px; border-radius: 50%; border: none; cursor: pointer;
  background: var(--border2); color: var(--text); display: flex; align-items: center; justify-content: center;
  flex-shrink: 0;
}
#preview-close-btn:hover { background: var(--border); }
#preview-body {
  flex: 1; overflow: auto; display: flex; align-items: center; justify-content: center;
  padding: 0; position: relative;
}
#preview-body img {
  max-width: 100%; max-height: 100%; object-fit: contain; border-radius: var(--r8);
  display: block;
}
#preview-body video, #preview-body audio {
  max-width: 100%; max-height: 100%; outline: none; border-radius: var(--r8);
}
#preview-body audio { min-width: 320px; margin: auto; }
#preview-body iframe {
...
#preview-unsupported .pu-icon { margin: 0 auto 18px; opacity: .5; }
#preview-unsupported h3 { font-size: 18px; font-weight: 600; color: var(--text2); margin-bottom: 8px; }
#preview-unsupported p { font-size: 14px; color: var(--text3); margin-bottom: 24px; }
#preview-spinner {
...
<div id="preview-overlay">
  <div id="preview-bar">
    ...
    <button id="preview-dl-btn" onclick="previewDownload()">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
      Download
    </button>
  </div>
  <div id="preview-body">
    <div id="preview-spinner" style="display:none">
      <svg width="38" height="38" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
    </div>
  </div>
</div>
...
function openPreview(id,mime,name){{
  ...
  // clear old content (keep spinner)
  ...
  if(m.includes('image')){{
    ...
    img.onerror=()=>{{hideSpinner();showUnsupported(name,IC.image);}};
    body.appendChild(img);
  }} else if(m.includes('video')){{
    const v=document.createElement('video');
    v.controls=true; v.autoplay=false; v.src=src; v.style.maxWidth='100%'; v.style.maxHeight='100%';
    v.oncanplay=hideSpinner;
    v.onerror=()=>{{hideSpinner();showUnsupported(name,IC.video);}};
    body.appendChild(v);
  }} else if(m.includes('audio')){{
    const a=document.createElement('audio');
    a.controls=true; a.src=src;
    a.oncanplay=hideSpinner;
    a.onerror=()=>{{hideSpinner();showUnsupported(name,IC.audio);}};
    body.appendChild(a);
    ...
    }}).catch(()=>{{hideSpinner();showUnsupported(name,IC.file);}});
  }} else {{
    hideSpinner();
    showUnsupported(name, getIco(m));
  }}
}}
...
function showUnsupported(name,icon){{
  ...
  d.innerHTML=`<div class="pu-icon">${{icon}}</div>
    <h3>Can't preview this file</h3>
    <p>No preview available for "${{name||'this file type'}}"</p>
    <button class="btn btn-primary" onclick="previewDownload()">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
      Download instead
    </button>`;
  body.appendChild(d);
