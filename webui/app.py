...
.vid-overlay-btn.hidden { display: none; }
...
function _buildVideoPlayer(body,onReady){{
  ...
  // Double-tap on video: left half = seek back 10s, right half = seek forward 10s
  // Single tap on video = do nothing (play/pause only via buttons)
...
function _vidFlash(playing){{
  ...
  void fl.offsetWidth; // reflow to restart animation
...
function _vidShowSeekRipple(side,secs){{
  ...
  // Remove existing ripple on same side
...
function _vidUpdateNavBtns(){{
  ...
  if(_vidList.length<=1){{
    if(pb)pb.classList.add('hidden');
    if(nb)nb.classList.add('hidden');
  }} else {{
    if(pb)pb.classList.toggle('hidden',_vidIdx===0);
    if(nb)nb.classList.toggle('hidden',_vidIdx===_vidList.length-1);
  }}
}}
