...
#video-player-wrap video {
  flex: 1; width: 100%; min-height: 0; display: block; cursor: pointer;
}
#vid-nav-overlay {
  position: absolute; inset: 0; display: flex; align-items: center; justify-content: center;
...
#vid-play-flash.flash { opacity: 1; transition: none; }
...
#vid-vol-slider {
  ...
  width: 70px; height: 4px; border-radius: 4px; outline: none; cursor: pointer;
...
function _getSiblingFiles(mime){{
  // Return files from current folder matching the same media type
  const m=mime||'';
...
function openPreview(id,mime,name){{
  ...
  if(m.includes('image')){{
    ...
    }}).catch(()=>{{hideSpinner();showUnsupported(name,getIco(m));}});
  }} else {{
...
function _buildVideoPlayer(body,onReady){{
  ...
  _vidEl=vid;
  wrap.innerHTML=`
    <div id="vid-nav-overlay"><div id="vid-play-flash"></div></div>
    <div id="vid-controls">
      ...
      <div id="vid-controls-row">
        <div class="vid-nav-btns">
          <button class="vid-btn" id="vid-prev-btn" title="Previous" onclick="vidNavigate(-1)">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6 8.5 6V6z"/></svg>
          </button>
          <button class="vid-btn" id="vid-play-btn" title="Play/Pause" onclick="vidTogglePlay()">
            <svg id="vid-play-ico" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
          </button>
          <button class="vid-btn" id="vid-next-btn" title="Next" onclick="vidNavigate(1)">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zm2-6 5.5-4v8L8 12zm8-6h2v12h-2z"/></svg>
          </button>
        </div>
        ...
        <div id="vid-vol-wrap">
          ...
          <input type="range" id="vid-vol-slider" min="0" max="1" step="0.02" value="1" oninput="vidSetVol(this.value)">
        </div>
  ...
  vid.oncanplay=()=>{{if(onReady)onReady();onReady=null;}};
  vid.onerror=()=>{{if(onReady)onReady();showUnsupported(f.name,IC.video);}};
  ...
  vid.addEventListener('click',vidTogglePlay);
  vid.addEventListener('dblclick',vidToggleFullscreen);
...
function _vidFlash(playing){{
  ...
  fl.classList.add('flash');setTimeout(()=>fl.classList.remove('flash'),400);
}}
function _vidUpdatePlayBtn(){{
  const ico=document.getElementById('vid-play-ico');if(!ico||!_vidEl)return;
  ico.innerHTML=_vidEl.paused
...
function _vidUpdateNavBtns(){{
  const pb=document.getElementById('vid-prev-btn');
  const nb=document.getElementById('vid-next-btn');
  if(pb)pb.disabled=_vidIdx===0;
  if(nb)nb.disabled=_vidIdx===_vidList.length-1;
  if(_vidList.length<=1){{if(pb)pb.style.visibility='hidden';if(nb)nb.style.visibility='hidden';}}
}}
  ...
  wrap.innerHTML=`
    ...
    <div id="audio-controls">
      ...
      <button class="aud-btn" onclick="audNavigate(1)" id="aud-next-btn" title="Next">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zm2-6 5.5-4v8L8 12zm8-6h2v12h-2z"/></svg>
      </button>
    </div>
...
function _audLoadTrack(onReady){{
  ...
  _audEl.oncanplay=()=>{{if(onReady){{onReady();onReady=null;}}}};
